...
| Offline LLM selection and configuration (edge-optimized) | LLM Interface via Ollama (Sec 4.2)   |
...
- **Offline LLM Inference:** Local execution via Ollama with lightweight models
  optimized for edge deployment (qwen3:0.6b default, CPU-only inference)
...
- **Containerized Deployment:** Docker Compose orchestration for edge devices
...
- Utilize a lightweight offline-capable LLM via Ollama optimized for edge
  devices
- Default model: qwen3:0.6b (~400MB, CPU-only inference)
- Alternative models: qwen3:1.7b, tinyllama, or equivalent small open-source
  models
- Optimized for resource-constrained edge/embedded device execution (no GPU
  required)
...
- Offline LLM operation via Ollama (no cloud API dependencies, CPU-only
  inference)
- Lightweight edge-optimized models (qwen3:0.6b default, ~400MB)
...
- Edge/embedded device deployment with minimal resource requirements (2-4GB RAM)
...
**LLM Performance Limitations:** Use Ollama with lightweight edge-optimized
models (qwen3:0.6b default, qwen3:1.7b fallback). Implement fallback mechanisms
for rule interpretation if LLM output is ambiguous or incorrect.

**Resource Constraints:** Profile and optimize critical code paths. Use
quantized models optimized for CPU inference via Ollama. Implement caching
strategies for repeated rule evaluations. Target 2-4GB total memory footprint.
